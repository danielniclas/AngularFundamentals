...
var app = angular.module('myApp', ['ngRoute']);
    ...
    app.factory('puppiesService', function ($http) {
        ...
        var puppies = [];                   //  This is the array I will build with the JSON ARRAY items
                                            //  How the array will be used in the HTML:
                                            //  First add $scope.puppies = puppiesService.get() to the CONTROLLER (above)
                                            //  <tr ng-repeat='pup in puppies' poopers-directive></tr>   ng-repeat='pup in puppies'

        return {                            //  Service RETURNS an OBJECT with one METHOD: get() METHOD
            get: function () {              //  get JSON and return  [{},{},{}] ARRAY of OBJECTS name puppies

                if (puppies.length == 0) {
                    $http.get('puppies.json')               //  Get puppies.json >>  [{},{},{}] ARRAY of OBJECTS
                        .success(function (response) {      //  'response' = puppies.json >>  [{},{},{}] ARRAY of OBJECTS

                            for (i=0; i<response.length; i++) {         //  Loop through the ARRAY and push Items into puppies ARRAY
                                var currentArrayItem = response[i];
                                puppies.push(currentArrayItem);         // push object into puppies [array] = [{},{},{}]
                            }
                        })
                        .error(function (err) {
                            alert('ERROR: ' + err)
    ...
    app.directive('poopersDirective', function () {         //  Return OBJECT (similar to Factory) >>  Directive Definition Object
        return {
            template:
            "<tr>" +
            "<td> {{pup.id}}</td>" +
            ...
            restrict: "AE",      //  Restrict how directive is used:  ELEMENT  "E", ATTRIBUTE  "A"
